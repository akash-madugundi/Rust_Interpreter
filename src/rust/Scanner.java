...
class Scanner {
    ...
    private void scanToken() {
        ...
        switch (c) {
            ...
            case '.': if(match('.')){addToken(DOT_DOT);} else {addToken(DOT); } break;
            ...
            default:
                if (isDigit(c)) {
                    ...
                    Rust.error(line, "Unexpected character.");
                }
                break;
        }
    }
    ...
    private void number() {
        boolean isFloat = false; // Flag to track if it's a float
        ...
        if (isFloat) {
            addToken(FLOAT, Float.parseFloat(numberLexeme));
        } else {
            addToken(INTEGER, Integer.parseInt(numberLexeme));
        }
    }
    ...
    private void string() {
        ...
        // Unterminated string.
        if (isAtEnd()) {
            Rust.error(line, "Unterminated string.");
            return;
        }
    ...
    private boolean match2(String expected) {
...
//    private String adv(){
//        current++;
//        return source.substring(current-1,current+7);
//    }
